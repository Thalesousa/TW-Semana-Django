...
from ..entidades.tarefas import Tarefa
...
def cadastrar_tarefas(request):
    if request.method == 'POST':
        ...
        if form_tarefa.is_valid():
            ...
            tarefanova = Tarefa(titulo = titulo, descricao = descricao, data_expiracao = data_expiracao,
                                prioridade = prioridade, usuario = request.user)
            tarefa_service.cadastrar_tarefa(tarefanova)
...
def remover_tarefa(request, id):
    ...
    if request.method == 'POST':
        ...
        return redirect('listar_tarefas')
    return render(request, 'tarefas/confirmar_exclusao.html', {'tarefa': tarefa_bd})